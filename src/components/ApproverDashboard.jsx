...
// Built-in Media Viewer (no external tab)
...
// Attachment block with built-in viewer and original filename display
    ...
    <div className="section-attachments">
      ...
      <div className="attachment-grid">
        {atts.map((a, i) => {
          ...
          // ✅ Use original_filename if available
...
export default function ApproverDashboard({ user, onLogout }) {
  ...
  const fetchPending = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/approver/pending');
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get('http://localhost:5000/api/approver/history');
  ...
  // ✅ FIX: Send attachment names to backend
  const handleApprove = async id => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/approver/approve/${id}`, fd, { headers:{'Content-Type':'multipart/form-data'} });
  ...
  const handleReject = async id => {
    ...
    try { await axios.put(`http://localhost:5000/api/approver/reject/${id}`,{approved_by:user.id,approver_comments:comments}); showToast('RMA rejected.','info'); closeModal(); fetchPending(); fetchHistory(); }
  ...
  const handleRequestChange = async id => {
    ...
    try { await axios.put(`http://localhost:5000/api/approver/request-change/${id}`,{approved_by:user.id,approver_comments:comments}); showToast('Changes requested.','info'); closeModal(); fetchPending(); fetchHistory(); }
    catch(e) { alert(e.response?.data?.error||'Failed'); }
  };

  // ✅ FIX: Send attachment names also on update
  const handleUpdateApproval = async () => {
    ...
    try {
      await axios.put(`http://localhost:5000/api/approver/update_approved/${selectedRMA.id}`, fd, { headers:{'Content-Type':'multipart/form-data'} });
        ...
        {activeTab==='history' && (
          <div className="panel">
            ...
            {loading ? <div className="loading-state">Loading…</div> : historyRmas.length===0 ? <div className="empty-state">No approval history yet.</div> : (
              <div className="table-wrap">
                <table className="data-table">
                  <thead><tr><th>RMA Number</th><th>Dealer</th><th>Status</th><th>Approved By</th><th>Date</th><th>Actions</th></tr></thead>
                  <tbody>
                    {historyRmas.map(r=>(
                      <tr key={r.id}>
                        <td className="td-rma">{r.rma_number}</td>
