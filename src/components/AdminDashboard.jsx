...
const STATUS_TEXT = { pending_dealer:'Pending Dealer', pending_authorizer:'Pending Auth', pending_approver:'Pending Approver', authorized:'Authorized', approved:'Approved', rejected:'Rejected' };
const STATUS_OPTS = [
  { value:'all', label:'All Status' }, { value:'pending_dealer', label:'Pending Dealer' },
  { value:'pending_authorizer', label:'Pending Auth' }, { value:'pending_approver', label:'Pending Approver' },
  { value:'authorized', label:'Authorized' }, { value:'approved', label:'Approved' }, { value:'rejected', label:'Rejected' },
];
const COMPANY = {
...
const EDIT_FIELDS = [
  { label:'Status', key:'status', type:'select', opts:[{v:'pending_dealer',l:'Pending Dealer'},{v:'pending_authorizer',l:'Pending Authorizer'},{v:'pending_approver',l:'Pending Approver'},{v:'authorized',l:'Authorized'},{v:'approved',l:'Approved'},{v:'rejected',l:'Rejected'}]},
  { label:'Return Type', key:'return_type', type:'select', opts:[{v:'Return Only',l:'Return Only'},{v:'Return for Credit',l:'Return for Credit'},{v:'Return for Exchange',l:'Return for Exchange'}]},
...
const downloadRMAPDF = async (rma) => {
  ...
  twoCol([['Filer Name',rma.filer_name],['Distributor',rma.distributor_name],['Dealer / Company',rma.company_name||rma.dealer_name]]);

  sectionTitle('Product & Return Details');
  twoCol([
    ['Product',rma.product],['Return Type',rma.return_type],
    ['Reason for Return',rma.reason_for_return],['Warranty',rma.warranty?'Yes – Under Warranty':'No – Out of Warranty'],
...
const downloadRMAExcel = (rma) => {
  const data = [{ 'RMA Number':rma.rma_number, 'Status':getStatusTxt(rma.status), 'Date Created':new Date(rma.created_at).toLocaleString(), 'Last Updated':new Date(rma.updated_at).toLocaleString(), 'Filer Name':rma.filer_name||'N/A', 'Distributor':rma.distributor_name||'N/A', 'Product':rma.product||'N/A', 'Product Description':rma.product_description||'N/A', 'Return Type':rma.return_type||'N/A', 'Reason':rma.reason_for_return||'N/A', 'Warranty':rma.warranty?'Yes':'No', 'Work Environment':rma.work_environment||'N/A', 'PO Number':rma.po_number||'N/A', 'Sales Invoice':rma.sales_invoice_number||'N/A', 'Shipping Date':rma.shipping_date||'N/A', 'Return Date':rma.return_date||'N/A', 'End User Company':rma.end_user_company||'N/A', 'Location':rma.end_user_location||'N/A', 'Industry':rma.end_user_industry||'N/A', 'Contact Person':rma.end_user_contact_person||'N/A', 'Problem Description':rma.problem_description||'N/A', 'Dealer Comments':rma.dealer_comments||'None', 'Authorized By':rma.authorized_by||'N/A', 'Authorized Date':rma.authorized_date||'N/A', 'Return Received By':rma.return_received_by||'N/A', 'Authorizer Comments':rma.authorizer_comments||'None', 'Approved By':rma.approved_by||'N/A', 'Approved Date':rma.approved_date||'N/A', 'Approved With':rma.approved_with||'N/A', 'Replacement / Credit Note':rma.replacement_order_no||'N/A', 'Closed Date':rma.closed_date||'N/A', 'Approver Comments':rma.approver_comments||'None' }];
...
const ChartCard = ({ title, wide, children }) => (
  <div className={`chart-card${wide?' chart-wide':''}`}>
    <div className="chart-title">{title}</div>{children}
  </div>
);
...
const STAT_CARDS = (rmas, stats, pd) => [
  ...
  { label:'Total Dealers',   value:stats.dealers?.total||0,                      accent:'#2563eb' },
  { label:'Pending Dealers', value:pd.length,                                    accent:'#8b5cf6' },
];
...
function AdminDashboard({ user, onLogout }) {
  ...
  useEffect(() => {
    let f = [...rmas];
    if (searchTerm) f = f.filter(r=>[r.rma_number,r.product_description,r.company_name,r.dealer_name,r.end_user_location].some(v=>(v||'').toLowerCase().includes(searchTerm.toLowerCase())));
  ...
  // ── Export ──
  const exportRMAsCSV = () => csvExport(filteredRmas.map(r=>({'RMA Number':r.rma_number,'Dealer':r.company_name||r.dealer_name,'Product':r.product_description,'Status':getStatusTxt(r.status),'Date Created':r.created_at?.split('T')[0],'PO Number':r.po_number||'','Return Type':r.return_type||'','Approved With':r.approved_with||'','Location':r.end_user_location||''})),'rma_export.csv');
  const exportDealersCSV = () => csvExport(allDealers.map(d=>({'Company':d.company_name,'Username':d.username,'Email':d.email,'Contact':d.contact_number,'City':d.city,'Barangay':d.barangay,'Status':d.is_approved?'Approved':'Pending','Registered':d.registered_at?.split('T')[0]})),'all_dealers.csv');
    ...
    <div className="admin-root">
      ...
      <div className="admin-container">
        ...
        <div className="tab-bar">
          {[{key:'overview',label:`All RMAs (${rmas.length})`},{key:'dealers',label:`Dealers (${allDealers.length})`},{key:'pending',label:`Pending Dealers (${pendingDealers.length})`},{key:'charts',label:'Analytics'}].map(t=>(
            ...
            <div className="panel-header">
              ...
              <div className="panel-actions">
                <div className="search-wrap">
                  ...
                  <input className="search-input" placeholder="Search RMA, product, dealer, location…" value={searchTerm} onChange={e=>setSearchTerm(e.target.value)}/>
                </div>
            ...
            {loading ? <div className="loading-state">Loading data…</div> : filteredRmas.length===0 ? <div className="empty-state">No RMA requests found.</div> : (
              <div className="table-wrap">
                <table className="data-table">
                  <thead><tr><th>RMA Number</th><th>Dealer</th><th>Product</th><th>Location</th><th>Status</th><th>Date</th><th>Actions</th></tr></thead>
                  <tbody>
                    {filteredRmas.map(r=>(
                      <tr key={r.id}>
                        <td className="td-rma">{r.rma_number}</td>
                        <td>{r.company_name||r.dealer_name}</td>
                        <td className="td-truncate">{truncStr(r.product_description||'',40)}</td>
                        <td>{truncStr(r.end_user_location||'',30)}</td>
        ...
        {activeTab==='dealers' && (
          <div className="panel">
            <div className="panel-header">
              <span className="panel-title">All Dealers</span>
              <div className="panel-actions"><button className="btn-export" onClick={exportDealersCSV}>Export CSV</button></div>
            </div>
            {allDealers.length===0 ? <div className="empty-state">No dealers found.</div> : (
              <div className="table-wrap">
                <table className="data-table">
                  <thead><tr><th>Company</th><th>Username</th><th>Email</th><th>Contact</th><th>Location</th><th>Status</th><th>Registered</th><th>Actions</th></tr></thead>
                  <tbody>
                    {allDealers.map(d=>(
                      <tr key={d.id}>
        ...
        {activeTab==='pending' && (
          <div className="panel">
            <div className="panel-header">
              <span className="panel-title">Pending Dealer Approvals</span>
              <div className="panel-actions"><button className="btn-export" onClick={exportDealersCSV}>Export CSV</button></div>
            </div>
            {pendingDealers.length===0 ? <div className="empty-state">No pending dealer approvals.</div> : (
              <div className="table-wrap">
                <table className="data-table">
                  <thead><tr><th>Company</th><th>Username</th><th>Email</th><th>Location</th><th>Registered</th><th>Actions</th></tr></thead>
                  <tbody>
        ...
        {activeTab==='charts' && (
          <div className="charts-grid">
            <ChartCard title="RMA Volume by Month" wide>
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={charts.monthly}>
                  <defs><linearGradient id="aG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={.25}/><stop offset="95%" stopColor="#2563eb" stopOpacity={0}/></linearGradient></defs>
            ...
            <ChartCard title="Avg. Resolution Time (Days)">
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={charts.avgResolution}>
                  <defs><linearGradient id="rG" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={.25}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient></defs>
            ...
            <ChartCard title="Top Dealers by RMA Count" wide>
              <ResponsiveContainer width="100%" height={220}>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box modal-large" onClick={e=>e.stopPropagation()}>
            ...
            <div className="detail-meta">
              ...
              <div><strong>Created:</strong> {new Date(selectedRMA.created_at).toLocaleString()}</div>
              <div><strong>Updated:</strong> {new Date(selectedRMA.updated_at).toLocaleString()}</div>
            ...
            <Section title="Filer Information">
              <Grid2><Field label="Filer Name" value={selectedRMA.filer_name}/><Field label="Distributor" value={selectedRMA.distributor_name}/><Field label="Dealer" value={selectedRMA.company_name||selectedRMA.dealer_name}/></Grid2>
            </Section>
            <Section title="Return Details">
              <Grid2>
                <Field label="Product" value={selectedRMA.product}/><Field label="Description" value={selectedRMA.product_description}/>
                <Field label="Return Type" value={selectedRMA.return_type}/><Field label="Reason" value={selectedRMA.reason_for_return}/>
                <Field label="Warranty" value={selectedRMA.warranty?'Yes':'No'}/><Field label="Work Env." value={selectedRMA.work_environment}/>
                <Field label="PO Number" value={selectedRMA.po_number}/><Field label="Sales Invoice" value={selectedRMA.sales_invoice_number}/>
                <Field label="Shipping Date" value={selectedRMA.shipping_date}/><Field label="Return Date" value={selectedRMA.return_date}/>
              </Grid2>
              <AttachmentBlock atts={selectedRMA.attachments} label="Dealer Attachments" cls="section-blue"/>
            </Section>
            <Section title="End User">
              <Grid2><Field label="Company" value={selectedRMA.end_user_company}/><Field label="Location" value={selectedRMA.end_user_location}/><Field label="Industry" value={selectedRMA.end_user_industry}/><Field label="Contact" value={selectedRMA.end_user_contact_person}/></Grid2>
            </Section>
            ...
            {selectedRMA.authorized_by && (
              <Section title="Authorization" accent="pink">
                <Grid2><Field label="Authorized By" value={selectedRMA.authorized_by}/><Field label="Authorized Date" value={selectedRMA.authorized_date}/><Field label="Return Received By" value={selectedRMA.return_received_by}/><Field label="Comments" value={selectedRMA.authorizer_comments||'None'}/></Grid2>
            ...
            {selectedRMA.approved_by && (
              <Section title="Approval" accent="green">
                <Grid2><Field label="Approved By" value={selectedRMA.approved_by}/><Field label="Approved Date" value={selectedRMA.approved_date}/><Field label="Approved With" value={selectedRMA.approved_with}/><Field label="Replacement / Credit Note" value={selectedRMA.replacement_order_no}/><Field label="Closed Date" value={selectedRMA.closed_date}/><Field label="Comments" value={selectedRMA.approver_comments||'None'}/></Grid2>
      ...
      {selectedDealer && dealerViewMode==='view' && (
        <div className="modal-overlay" onClick={closeDealerModal}>
          <div className="modal-box modal-medium" onClick={e=>e.stopPropagation()}>
            <div className="modal-head">
              <h2>Dealer — {selectedDealer.company_name||selectedDealer.username}</h2>
              <button className="modal-close" onClick={closeDealerModal}>×</button>
            </div>
            <Section title="Account">
              <Grid2><Field label="Username" value={selectedDealer.username}/><Field label="Email" value={selectedDealer.email}/><Field label="Contact" value={selectedDealer.contact_number}/><Field label="Status" value={selectedDealer.is_approved?'Approved':'Pending Approval'}/><Field label="Registered" value={selectedDealer.registered_at?new Date(selectedDealer.registered_at).toLocaleString():'N/A'}/></Grid2>
            </Section>
            <Section title="Company">
              <Grid2><Field label="Company Name" value={selectedDealer.company_name}/><Field label="City" value={selectedDealer.city}/><Field label="Barangay" value={selectedDealer.barangay}/></Grid2>
            </Section>
            <div className="modal-footer">
      ...
      {selectedDealer && dealerViewMode==='edit' && (
        <div className="modal-overlay" onClick={closeDealerModal}>
          <div className="modal-box modal-medium" onClick={e=>e.stopPropagation()}>
            <div className="modal-head">
              <h2>Edit Dealer — {selectedDealer.company_name||selectedDealer.username}</h2>
              <button className="modal-close" onClick={closeDealerModal}>×</button>
            </div>
            <div className="form-body">
