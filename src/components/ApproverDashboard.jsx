...
// ─── Constants ────────────────────────────────────────────────────────────────
const COMPANY = {
  name: 'DELTAPLUS PHILIPPINES, INC.',
  vat: 'VAT REG. TIN-009-277-410-00000',
...
const APPROVED_WITH_OPTS = [
  ...
  { value: 'Replacement Unit', label: 'Replacement Unit' },
  ...
  return (
    <div className="section-attachments">
      ...
      <div className="attachment-grid">
        {atts.map((a, i) => {
          const isImg = a.resource_type === 'image';
          const isVid = a.resource_type === 'video';
          const mediaIdx = mediaItems.findIndex(m => m.url === a.url);
          const displayName = a.original_filename || a.filename || getFallback(a.url);
...
// ─── Preview Strip Component (for upload) ─────────────────────────────────────
    ...
    {previews.map((p, i) => (
      <div key={i} className="preview-item">
        {p.type?.startsWith('image/') ? (
          ...
          <video src={p.url} muted style={{ width: '100%', height: '80px', objectFit: 'cover' }} />
        ) : (
          <div className="file-preview-box">
            <span className="file-icon-large">{fileIcon(p.type)}</span>
            <span className="file-name-small">{p.name?.substring(0, 15)}</span>
          </div>
        )}
...
const downloadRMAPDF = async (rma) => {
  ...
  [COMPANY.vat, COMPANY.sub, COMPANY.address, COMPANY.tel].forEach(line => {
    doc.text(line, lm, y);
    y += 4;
  });
  doc.setTextColor(37, 99, 235);
  doc.text(COMPANY.web, lm, y);
  ...
  const sectionTitle = title => {
    ...
    doc.roundedRect(lm, y, pw - lm - rm, 7, 1, 1, 'F');
    doc.setTextColor(255, 255, 255);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    doc.text(title.toUpperCase(), lm + 4, y + 4.8);
    y += 10;
  };
  ...
  const twoCol = pairs => {
    ...
    pairs.forEach((pair, i) => {
      ...
      doc.roundedRect(x, y, colW, 8.5, 1, 1, 'F');
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(6.5);
      doc.setTextColor(100, 116, 139);
  ...
  const fullRow = (label, value) => {
    ensureSpace(14);
    doc.setFillColor(241, 245, 249);
    doc.roundedRect(lm, y, pw - lm - rm, 11, 1, 1, 'F');
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(6.5);
    doc.setTextColor(100, 116, 139);
  ...
  twoCol([
    ['Product', rma.product],
    ['Return Type', rma.return_type],
    ['Reason for Return', rma.reason_for_return],
    ['Warranty', rma.warranty ? 'Yes – Under Warranty' : 'No – Out of Warranty'],
    ['Work Environment', rma.work_environment],
    ['PO Number', rma.po_number],
    ['Sales Invoice No.', rma.sales_invoice_number],
  ...
  twoCol([
    ['Company', rma.end_user_company],
    ['Location', rma.end_user_location],
    ['Industry', rma.end_user_industry],
    ['Contact Person', rma.end_user_contact_person]
  ]);
  ...
  if (rma.authorized_by) {
    ...
    twoCol([
      ['Authorized By', rma.authorized_by],
      ['Authorized Date', rma.authorized_date],
      ['Return Received By', rma.return_received_by],
  ...
  if (rma.approved_by) {
    ...
    twoCol([
      ['Approved By', rma.approved_by],
      ['Approved Date', rma.approved_date],
      ['Approved With', rma.approved_with],
      ['Replacement / Credit Note', rma.replacement_order_no],
      ['Closed Date', rma.closed_date],
...
const exportTableExcel = (data, filename, sheetName) => {
  ...
  const ws = XLSX.utils.json_to_sheet(data.map(r => ({
    ...
    'Dealer': r.company_name || r.dealer_name,
    'Product': r.product_description,
    'Status': getStatusTxt(r.status),
    'Authorized By': r.authorizer_name || r.authorized_by || 'N/A',
...
export default function ApproverDashboard({ user, onLogout }) {
  ...
  const [approveData, setApproveData] = useState({
    approved_by: 'Michael John Canatoy',
    approved_date: new Date().toISOString().split('T')[0],
    approved_with: APPROVED_WITH_OPTS[0].value,
  ...
  const fetchPending = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/approver/pending`);
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/approver/history`);
  ...
  const openEditModal = rma => {
    setSelectedRMA(parseRMA(rma));
    setViewMode('edit');
    setEditData({
      approved_by: rma.approved_by || '',
      approved_date: rma.approved_date || '',
      approved_with: rma.approved_with || APPROVED_WITH_OPTS[0].value,
      replacement_order_no: rma.replacement_order_no || '',
      closed_date: rma.closed_date || '',
      approver_comments: rma.approver_comments || ''
    });
    setEditAtts([]);
    setEditPreviews([]);
  };
  ...
  const handleApprove = async id => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/api/approver/approve/${id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
      showToast('RMA approved successfully.', 'success');
      closeModal();
      setApproverAtts([]);
      setAttPreviews([]);
      fetchPending();
  ...
  const handleReject = async id => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/api/approver/reject/${id}`, { approved_by: user.id, approver_comments: comments });
      showToast('RMA rejected.', 'info');
      closeModal();
      fetchPending();
      fetchHistory();
  ...
  const handleRequestChange = async id => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/api/approver/request-change/${id}`, { approved_by: user.id, approver_comments: comments });
  ...
  const handleUpdateApproval = async () => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/api/approver/update_approved/${selectedRMA.id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
    ...
    <div className="dash-root">
      ...
      <div className="dash-container">
        ...
        <div className="stats-row stats-row--2">
          <div className="stat-card">
            <div className="stat-accent" style={{ backgroundColor: '#8b5cf6' }} />
            <div className="stat-label">Pending Approval</div>
            <div className="stat-value" style={{ color: '#8b5cf6' }}>{pendingRmas.length}</div>
          </div>
          <div className="stat-card">
            <div className="stat-accent" style={{ backgroundColor: '#1e3a5f' }} />
            <div className="stat-label">Total Processed</div>
            <div className="stat-value" style={{ color: '#1e3a5f' }}>{historyRmas.length}</div>
          </div>
        </div>
        ...
        <div className="tab-bar" style={{ justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <div style={{ display: 'flex', gap: '4px' }}>
            <button className={`tab-btn${activeTab === 'pending' ? ' active' : ''}`} onClick={() => setActiveTab('pending')}>
              Pending Approval ({pendingRmas.length})
            </button>
            <button className={`tab-btn${activeTab === 'history' ? ' active' : ''}`} onClick={() => setActiveTab('history')}>
              Approval History ({historyRmas.length})
            </button>
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button className="btn-export" onClick={() => exportTableExcel(activeTab === 'pending' ? pendingRmas : historyRmas, activeTab === 'pending' ? 'Pending_Approvals' : 'Approval_History', activeTab === 'pending' ? 'Pending Approvals' : 'Approval History')}>
              📎 Export Excel
            </button>
          </div>
        </div>
            ...
            {pendingRmas.length === 0 ? <div className="empty-state">No pending approvals.</div> : (
              <div className="table-wrap">
                <table className="data-table">
                  <thead>
                    <tr><th>RMA Number</th><th>Dealer</th><th>Product</th><th>Authorized By</th><th>Actions</th></tr>
                  </thead>
                  <tbody>
                    {pendingRmas.map(r => (
                      <tr key={r.id}>
          ...
          <div className="panel">
            ...
            {loading ? <div className="loading-state">Loading…</div> : historyRmas.length === 0 ? <div className="empty-state">No approval history yet.</div> : (
              <div className="table-wrap">
                <table className="data-table">
                  <thead>
                    <tr><th>RMA Number</th><th>Dealer</th><th>Status</th><th>Approved By</th><th>Date</th><th>Actions</th></tr>
                  </thead>
                  <tbody>
                    {historyRmas.map(r => (
                      <tr key={r.id}>
        ...
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box modal-large" onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              <h2>RMA Details — {selectedRMA.rma_number}</h2>
      ...
      {selectedRMA && viewMode === 'approve' && (
        <div className="modal-overlay" onClick={() => setSelectedRMA(null)}>
          <div className="modal-box modal-large" onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              <h2>Approve RMA — {selectedRMA.rma_number}</h2>
              <button className="modal-close" onClick={() => setSelectedRMA(null)}>×</button>
            </div>
            <div className="detail-section">
      ...
      {selectedRMA && viewMode === 'edit' && (
        <div className="modal-overlay" onClick={closeModal}>
          <div className="modal-box modal-medium" onClick={e => e.stopPropagation()}>
            <div className="modal-head">
              <h2>Edit Approval — {selectedRMA.rma_number}</h2>
              <button className="modal-close" onClick={closeModal}>×</button>
            </div>
