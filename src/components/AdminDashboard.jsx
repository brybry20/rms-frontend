...
const downloadRMAPDF = async (rma) => {
  ...
  twoCol([['Filer Name', rma.filer_name], ['Distributor', rma.company_name || rma.distributor_name]]);
...
const downloadRMAExcel = (rma) => {
  const data = [{ 
    ...
    'Distributor': rma.company_name || rma.distributor_name || 'N/A', 
...
function AdminDashboard({ user, onLogout }) {
  ...
  const exportRMAsCSV = () => csvExport(filteredRmas.map(r => ({
    ...
    'Distributor': r.company_name || r.distributor_name || 'N/A',
  ...
  const exportDealersCSV = () => csvExport(allDealers.map(d => ({
    ...
    'Status': d.is_approved === 1 ? 'Approved' : 'Pending',
      ...
      <div className="table-wrap">
        <table className="data-table">
          ...
          <tbody>
            {allDealers.map(d => (
              <tr key={d.id}>
                <td>{d.profile?.company_name || d.company_name || 'N/A'}</td>
                <td>{d.username}</td>
                <td>{d.email}</td>
                <td>{d.contact_number || 'N/A'}</td>
                <td>
                  {d.profile?.is_approved === 1 ? 
                    <span className="status-badge status-approved">Approved</span> : 
                    <span className="status-badge status-pending_dealer">Pending</span>
                  }
                </td>
                <td>
                  <div className="action-btns">
                    ...
                    {d.profile?.is_approved !== 1 && <button className="btn-action btn-approve" onClick={() => approveDealer(d.id)}>Approve</button>}
          ...
          <div className="modal-box modal-large" onClick={e => e.stopPropagation()}>
            ...
            <Section title="Filer Information">
              <Grid2>
                ...
                <Field label="Distributor" value={selectedRMA.company_name || selectedRMA.distributor_name} />
              </Grid2>
            </Section>
          ...
          <div className="modal-box modal-medium" onClick={e => e.stopPropagation()}>
            ...
            <Section title="Account">
              <Grid2>
                ...
                <Field label="Status" value={selectedDealer.is_approved === 1 ? 'Approved' : 'Pending Approval'} />
